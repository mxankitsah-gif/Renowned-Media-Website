...
export const BuilderToolbar: React.FC = () => {
  ...
  return (
    <>
      ...
      {toastMessage && (
        <div className="fixed top-20 right-6 z-[90] bg-slate-900 text-white px-5 py-3 rounded-2xl shadow-2xl border border-blue-500/50 flex items-center gap-3 animate-bounce font-mono text-xs font-bold">
          <Sparkles className="w-5 h-5 text-blue-400 shrink-0" />
          <span>{toastMessage}</span>
        </div>
      )}

      {/* Floating WordPress Builder Control Panel (ONLY when Admin is Logged In) */}
      {isAdminLoggedIn && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[90] max-w-4xl w-[92%] sm:w-auto">
          <div className="bg-slate-900/95 text-white backdrop-blur-xl border border-slate-700/80 rounded-2xl p-2.5 shadow-[0_12px_40px_rgba(0,0,0,0.5)] flex flex-wrap items-center justify-between gap-3 text-xs font-mono">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 px-3 py-1.5 rounded-xl font-extrabold text-white">
                ...
                <span className="hidden sm:inline">WP-Admin Active</span>
            ...
            {/* Last Saved Badge & Logout Button */}
            <div className="flex items-center gap-2">
              {lastSavedAt && (
              ...
              <button
                onClick={logoutAdmin}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-red-900/60 text-slate-300 hover:text-red-300 rounded-xl transition-colors cursor-pointer border border-slate-700"
                title="Exit Admin Backend"
              >
